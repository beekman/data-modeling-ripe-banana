...
module.exports = async ({
  reviewers = 20, actors = 50, studios = 50, reviews = 50, films = 50 } = {}) => {
  const reviewers = await Reviewer.create([...Array(reviewers)].map(() => ({
    name: chance.name({ type: 'adult' }),
    company: chance.company()
  })));


  const actors = await Actor.create([...Array(actors)].map(() => ({
    name: chance.name(),
    dob: chance.birthday(),
